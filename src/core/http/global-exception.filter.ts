...
export class GlobalExceptionFilter implements ExceptionFilter {
  ...
  private resolve(exception: unknown): {
    ...
    if (exception instanceof Prisma.PrismaClientKnownRequestError) {
      ...
      if (
        ['P2000', 'P2003', 'P2011', 'P2012', 'P2019', 'P2020', 'P2023'].includes(exception.code)
      ) {
        return {
    ...
    // BUG FIX (defense in depth): thrown whenever a value reaches Prisma
    // that doesn't fit the schema shape it's querying against — e.g. a
    // string that isn't a valid Prisma enum member. Confirmed reachable via
    // POST/DELETE /owner/role-permissions before their DTO/param validation
    // was tightened (see owner-governance.dto.ts, role-permissions.controller.ts);
    // handled here too in case another unvalidated enum-typed field exists
    // elsewhere, so a Prisma-shape mismatch is still a 400, not a 500.
