...
| Method | Endpoint | Purpose / actor and scope | DTO / key errors | Tx / idempotency | FE consumers |
|---|---|---|---|---|---|
| POST | `/auth/login`, `/auth/refresh`, `/auth/logout` | session lifecycle | credentials/session; AUTH_* | refresh rotation tx | Login/header |
| POST | `/auth/password/forgot`, `/reset` | reset flow | email/token/password | idempotent response | future login link |
| GET/PATCH | `/me`, `/me/preferences` | own profile/preferences | preference DTO | versioned | header/profile/settings |
| GET/PATCH | `/patients/{id}` | scoped profile | patient DTO | audit sensitive read | Profile/staff detail |
| GET/POST | `/patients/{id}/consents`, `.../{type}/grant|withdraw` | consent history/command | type/version | tx + idempotent | Settings |
| GET | `/practitioners`, `/availability` | searchable doctor/slots | specialty, clinic, date | read | Appointments |
| GET/POST | `/appointments`, `/appointments/{id}` | list/book/view | booking DTO | booking tx + key | Appointments/detail |
| POST | `/appointments/{id}/cancel`, `/reschedule`, `/mark-missed` | explicit transitions | reason/slot | tx + key | booking/reception |
| POST | `/appointments/{id}/check-in-token`, `.../revoke` | issue/revoke QR | reason/version | tx | QR component |
| POST | `/check-ins` | redeem QR | token, clinic, device | tx + key | kiosk |
| GET | `/queue/tickets`, `/queue/stations`, `/queue/public-feed-token` | queue/list/reference | clinic/station/status | read | queue pages |
| POST | `/queue/tickets/{id}/call|acknowledge|start|skip|complete|route` | queue commands | station/reason/version | tx + key | ClinicQueue |
| GET/POST | `/encounters`, `/encounters/{id}` | scoped list/detail/create | filters/create DTO | create tx | Journey/CRM |
| POST | `/encounters/{id}/transitions/{command}` | guarded encounter command | reason/version | tx + key | services/workflows |
| GET/POST | `/encounters/{id}/intakes` | list/submit intake | intake DTO/file IDs | tx + key | AIAnalysis |
| GET/POST | `/encounters/{id}/ai-assessments`, `.../reassess` | list/generate | intake version | job/tx + key | AIAnalysis/DoctorReview |
| POST | `/ai-assessments/{id}/reviews` | doctor review | action/code/rationale | tx + key | DoctorReview |
| GET/POST | `/encounters/{id}/diagnoses` | list/create | status/code/name | tx | DoctorReview/records |
| POST | `/diagnoses/{id}/revise` | append revision | rationale/new diagnosis | tx + key | DoctorReview |
| POST | `/encounters/{id}/clinical-plans` | approve plan | diagnosis/summary | tx + key | DoctorReview |
| GET/POST | `/encounters/{id}/orders` | list/create | type/justification/role | tx + key | DoctorReview/Records |
| POST | `/orders/{id}/start|invalid-sample|complete|cancel` | order commands/result | reason/result DTO | tx + key | workflow/Records |
| GET/POST | `/workflow-templates`, `/{id}/drafts` | list/create/draft | metadata/base version | tx | template pages |
| PATCH | `/workflow-templates/{id}`, `/workflow-versions/{id}/layout` | details/layout | version/patch | optimistic | editor |
| POST | `/workflow-versions/{id}/steps|edges`, `.../validate|submit-review|publish|archive` | graph/version commands | step/edge/reason | tx; publish key | editor |
| GET/POST | `/workflow-instances`, `/encounters/{id}/workflow-instances` | list/activate | template ID | tx + key | editor/runtime |
| GET | `/workflow-instances/{id}`, `/work-queue` | runtime/task lists | filters/cursor | read | instance/queue |
| POST | `/workflow-tasks/{id}/accept|start|complete|redo|reject|escalate|skip|reassign` | task commands | reason/assignee/version | tx + key | work queue/instance |
| POST | `/workflow-instances/{id}/suspend|resume|cancel|complete` | instance commands | reason/version | tx + key | instance |
| GET | `/encounters/{id}/medical-record` | scoped record | projection | audited read | Records |
| POST | `/encounters/{id}/medical-record/draft|diagnosis|discharge-follow-up` | record assembly | typed DTOs | tx | Records |
| POST | `/medical-records/{id}/sign|addenda|reopen` | immutable lifecycle | reason/text/version | tx + key | Records |
| GET/POST | `/encounters/{id}/prescriptions` | list/issue | items/note | tx + key | Doctor/Prescriptions |
| POST | `/files/upload-intents`, `/files/{id}/complete` | private upload | category/MIME/size/checksum | key | AI/progress/records/profile |
| POST/GET | `/encounters/{id}/documents`, `/documents/{id}/download` | attach/list/download | metadata | tx; audit | Records |
| POST | `/documents/{id}/review|flag-incorrect` | document review | reason/version | tx | Records |
| GET/POST | `/patients/{id}/care-plans`, `/care-plans/{id}/activities` | care data | activity DTO | tx | Care |
| POST | `/care-activities/{id}/confirm|transition`, `/care-plans/{id}/run-automation` | follow-up commands | state/version | tx/job + key | Care |
| GET/POST | `/clinical-alerts`, `/care-plans/{id}/alerts` | list/raise | trigger/note | tx + key | Care/dashboard |
| POST | `/clinical-alerts/{id}/acknowledge|resolve`, `/encounter-requests/{id}/approve|reject` | alert/request decisions | reason/version | tx + key | Care |
| GET/POST | `/notifications`, `/notifications/read-all`, `/{id}/read|retry` | notification inbox | cursor | tx/job | TopHeader |
| GET | `/audit-events` | filter audit | actor/action/resource/patient/date/cursor | read | AuditViewer |
| GET/POST | `/integrations`, `/integrations/{id}/retry-failed|reset-dlq` | operations | reason | job + key | Integrations |
| GET/POST | `/patients/{id}/progress`, `/patients/{id}/progress-assessments` | tracking/upload analysis | file/note | job + key | Progress |
| GET | `/patients/{id}/reports/summary`, `/reports/{type}.pdf` | report data/export | period/type | generated job/read | Reports |
| POST | `/support-tickets` | submit support request | topic/message | key | Support |
| GET | `/health/live`, `/health/ready` | operations | none | read | deployment |

Delete endpoints are deliberately absent for signed/clinical resources. Draft workflow step removal is a command under version steps and must reject mandatory/in-use content.
...
```json
{"data":{"id":"uuid","code":"APT-2042","status":"upcoming","startsAt":"2026-07-20T02:00:00Z","endsAt":"2026-07-20T02:30:00Z","clinic":{"id":"uuid","name":"DermaHealth TP.HCM"},"practitioner":{"id":"uuid","name":"Bs. Nguyễn Thị An"},"checkIn":{"qrToken":"returned-once","validFrom":"...","expiresAt":"..."}},"meta":{},"requestId":"uuid"}
```
...
```json
POST /api/v1/medical-records/{id}/sign
Idempotency-Key: ...
...
| Frontend source/current symbol | Replacement | Required frontend change |
|---|---|---|
| `store.ts`, `repositories.ts` all stores | API client + TanStack Query | remove domain localStorage authority; keep temporary adapters during migration |
| `AppStateContext.tsx` user selector/session ID | `/auth/*`, `/me` | real session bootstrap; dev role switch only behind explicit development endpoint/build flag |
| `patientService` | `/me`, `/patients/*`, consent commands | canonical DTOs and scoped errors |
| `appointmentService`, inline doctors/slots | practitioners/availability/appointment commands | submit `slotId`; preserve reason; show conflict; invalidate appointment/queue queries |
| `checkInService`, QR component | token/check-in endpoints | raw token only in QR payload; generic validation errors; kiosk device credential |
| `queueService` | queue query/commands + SSE | command mutations, version conflicts, realtime cache updates |
| `encounterService` | encounter detail/command endpoints | no arbitrary local transition; map labels client-side or use enum |
| `aiAssessmentService` | intake/assessment endpoints | multipart becomes upload-intent + file ID; handle 202/poll/SSE and red flags |
| doctor/order services | review/diagnosis/plan/order/result commands | selected encounter IDs from scoped API; stable error display |
| `workflowService` | workflow template/runtime endpoints | server IDs/version; optimistic lock; cache template/version/instance/task keys |
| `medicalRecordService` | records/prescriptions/files commands | real upload; patient-visible projection; signing attestation/version |
| `crmService` | care/alerts/request endpoints | role-scoped lists; background automation status |
| `notificationService` | notification inbox/read/retry | remove random delivery; cursor pagination and unread cache |
| `auditService` | audit endpoint | server filters/cursor; never client log events |
| `Integrations.tsx` direct upserts | integration command endpoints | no direct “delivered” status mutation |
| `mockData.ts` prescription/progress/report arrays | prescriptions/progress/report APIs | canonical item durations/timestamps; derived charts |
| Profile/Records/Dashboard inline data | scoped aggregate endpoints | remove duplicates and volatile Kanban; map treatment tasks to follow-up activities |
| Settings/Support local controls | preferences/consents/support APIs | form validation, loading/success/failure; logout endpoint |
